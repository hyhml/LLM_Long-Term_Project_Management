...
user mode: global total skill
  thin entry + generator + shared tools + compatibility/migration service
                              |
                              | confirmed project specification
                              v
project-local child skill
  project adapter + framework lock + project contract/data/map/work/packages
```
...
A child skill is one managed project instance, not an independent fork of the total skill. Project data travels with the project. The child must remain identifiable, validatable, and upgradeable without silently changing accepted project facts.
...
| `PROD-005` | high | Project data and its child skill travel with the project. | partial | Project-local layout exists; cross-machine compatibility and upgrade lifecycle are incomplete. |
| `PROD-006` | high | Treat the child skill as a managed project instance rather than an independently diverging framework fork. | planned | Managed-file ownership and framework lock are not implemented. |
...
| `PROD-008` | high | Divide framework rules into high- and low-load priorities; load the binding core directly and conditional detail only when triggered. | implemented | Runtime framework map, total skill router, and child conditional modules. |
...
| `CHILD-004` | high | Record the total-framework release, child-template version, storage schemas, compatibility range, and managed-file hashes in the child. | planned | Only a basic `framework/version.json` snapshot exists. |
| `CHILD-005` | high | When the total skill updates, detect each opened child's version and supported compatibility before allowing formal writes. | planned | No compatibility checker exists. |
| `CHILD-006` | high | Provide a supported child-architecture synchronization path after total-skill updates. | open-design | Required outcome is confirmed. Automatic detection versus user-confirmed application, version retention, and fallback behavior require explicit design. |
| `CHILD-007` | high | Upgrade framework-managed child files without overwriting project data or unrecognized user modifications. | planned | No managed-file manifest or three-way conflict handling exists. |
| `CHILD-008` | high | Split child architecture upgrades (`framework`) from project-data schema migrations (`data:project`); use proposal, validation, revision, and receipts for each boundary. | planned | Existing project transaction covers accepted data writes, but no child upgrader/migrator exists. |
| `CHILD-009` | high | Permit read-only inspection when compatibility is uncertain; block unsafe formal writes rather than guessing. | planned | No compatibility gate exists. |
...
| ID | Priority | Requirement | Status | Evidence or gap |
|---|---|---|---|---|
| `QUAL-001` | high | Protect six core invariants: cross-conversation loop, authority boundary, decision/revision transaction, project identity, rebuildability, and information boundary. | implemented | 23 core tests. |
...
| `ENV-006` | high | Support project transfer between systems without assuming shared state; use portable project data and single-file handoffs with integrity checking. | partial | Handoffs are portable; child-framework version compatibility across machines is incomplete. |
...
2. **Child version lock and synchronization (`CHILD-004`–`CHILD-009`)**: the current child stores only a basic framework version and has no compatibility check, managed-file ownership, upgrader, or data-migration workflow.
3. **Cross-machine continuity (`ENV-006`)**: handoff transport works, but a moved child may depend on a different installed total-skill version without a compatibility gate.
4. **Parallel exploration (`FLOW-007`)**: high priority but intentionally deferred; stale transactions are rejected, while parallel rebase/merge is not implemented.
5. **Machine-profile first-run orchestration (`ENV-003`)**: the tool exists, but the end-to-end user interaction needs more validation.
...
1. When a compatible total-skill update is detected, should child managed files update automatically, require one batch confirmation, or update only on explicit request?
2. Should a child carry a minimal executable fallback runtime for offline/old-version use, or require a compatible globally installed total skill?
3. How many historical framework versions, if any, should the total skill support side by side?
4. Where should user-authored project-specific instructions live so managed template files can be safely regenerated?
5. Should local project discovery use an optional private registry, explicit roots, or both?
6. What is the rollback guarantee for a child architecture upgrade that succeeds before a separate project-data migration fails?
...
4. state the acceptance evidence and current gap;
5. read relevant ADRs and policies;
6. update the product map if the user's requirement, priority, scope, or status changed;
7. implement and test without declaring adjacent requirements complete;
8. reconcile requirement status with actual evidence before release.
