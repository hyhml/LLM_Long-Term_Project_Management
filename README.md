...
一个面向 Codex CLI 的显式调用 skill，用项目本地正式记录、派生项目地图、探索交接包和用户逐项确认，实现跨对话协作与长期项目管理。
...
1. 整理对话维护正式记录、类型化关系和已确认待办，再生成项目地图。
2. 探索对话只执行一个已确认任务，未确认内容留在 `work/`，不修改 `records/` 或 `views/`。
3. 探索对话导出单文件 `.llmpack`。
4. 整理对话校验并解包，逐项提出变更。
