...
2. At the start of every substantive managed request, state a concise two-axis classification in the conversation: change layer (`project-data`, `framework`, `mixed`, or `none/read-only`) and audience (`runtime-user`, `developer`, or `both`). Classify by the authority actually changed, not by subject matter or which tool is used. If mixed, split the work and release boundaries before editing. Repeat the classification in the final handoff when changes were made.
...
13. Keep the private machine profile outside exports. Never record secrets, tokens, raw credentials, or unrelated private paths.
...
- If either classification axis is ambiguous, mixed, or changes during work, read `references/change-classification.md` before proceeding.
